...
extension RuntimeSensitiveTests {
    ...
    struct GitErrorTests {
        ...
        @Test
        func fromLibgit2ProducesUnknownWhenNoErrorIsSet() throws {
            // libgit2 intentionally fails allocations (including git_error_last's
            // TLS storage) until it is initialized, so make sure we're bootstrapped.
            try Git.bootstrap()
        ...
        @Test
        func checkThrowsOnFailure() throws {
            // libgit2 intentionally fails allocations until it is initialized.
            try Git.bootstrap()
            defer { try? Git.shutdown() }
