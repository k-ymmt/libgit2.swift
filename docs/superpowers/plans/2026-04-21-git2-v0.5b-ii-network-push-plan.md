...
synthesizes a single .user/.reference GitError from the collected
...
## Task 6: Non-fast-forward rejection surfaces as `GitError(.user, .reference)`
...
```bash
git add Tests/Git2Tests/Remote/RemotePushTests.swift
git commit -m "test(push): non-fast-forward surfaces as GitError(.user, .reference)"
```
...
```swift
    ...
    // NOTE: pushUpdateReference_firesOnRejectedRef is NOT landed here.
    // Empirically, libgit2's file:// transport rejects non-fast-forward
    // directly from git_remote_push with GIT_ENONFASTFORWARD — it does
    // NOT invoke push_update_reference for rejection. Over file://,
    // push_update_reference only fires with status == nil (accepted).
    // The rejected-ref-callback path is exercised by Task 12's GitHub
    // integration test `push_nonFastForward_throwsCallbackReference`
    // which extends the assertion with a CollectedUpdates check to
    // prove pushUpdateReference fired with status != nil on the HTTPS
    // transport.
```
...
Run: `swift test --filter RemotePushCallbackTests`
Expected: PASS (3 cases total — `pushTransferProgress_firesAtLeastOnce`, `pushTransferProgress_returningFalseCancelsWithUserError`, `pushUpdateReference_firesOnAcceptedRef`).

- [ ] **Step 3: Commit**

```bash
git add Tests/Git2Tests/Remote/RemotePushCallbackTests.swift
git commit -m "test(push): pushUpdateReference fires for accepted refs (file://)"
```
...
Also append the collector helper at the bottom of
`Tests/Git2Tests/Remote/RemoteGitHubIntegrationTests.swift` (after the
existing `private final class Counters`):
...
(`import os` may already be present in the file — keep a single import at
the top of the file; the block shown is the helper itself.)
...
2. push_nonFastForward_throwsCallbackReference — server reject. Also
   verifies pushUpdateReference fires with status != nil (HTTP-only
   signal; not reachable over file:// per empirical libgit2 behavior).
...
- `Remote.push(refspecs:options:)` — wraps `git_remote_push`. Per-ref server rejections (non-fast-forward, pre-receive hook, protected branches) are collected during the call and surfaced as a single `GitError(code: .user, class: .reference)` with a semicolon-delimited `refname: status` message. (`.user` parallels fetch's `GIT_EUSER` mapping; `.callback` is not a `GitError.Code` case.) Callers that want programmatic per-ref results set `options.pushUpdateReference`.
